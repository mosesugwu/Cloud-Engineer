...
echo "Ensuring runner directory exists"
mkdir -p "$RUNNER_DIR"
cd "$RUNNER_DIR" || { echo "Failed to change directory to $RUNNER_DIR"; exit 1; }
...
# Ensure correct ownership before configuration
sudo chown -R $USER:$USER "$RUNNER_DIR"

# Run configuration as user
sudo -u $USER bash -c "cd $RUNNER_DIR && ./config.sh --url https://github.com/$OWNER/$REPO --token $RUNNER_TOKEN --name ghrunner-vm02 --labels self-hosted,Linux,X64,ghrunner-vm02 --runnergroup Default"
